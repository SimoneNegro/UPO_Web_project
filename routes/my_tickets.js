...
router.get('/', function (req, res, next) {
    if (!req.user) { res.render('my_tickets', { title: "My Tickets", user: req.user }); }
    ...
    db.get('SELECT COUNT(*) AS num_user_ticket FROM ticket t INNER JOIN utente u ON t.id_utente = u.id WHERE u.id = ?', [req.user.id], function (err, row) {
        ...
        if (row.num_user_ticket == 0) { res.render('my_tickets', { title: "My Tickets", user: req.user, num_tickets: row }); }

        // get all user tickets
        db.all('SELECT * FROM ticket WHERE id_utente = ?', [req.user.id], function (err, rows) {
            res.render('my_tickets', { title: "My Tickets", user: req.user, num_tickets: row, tickets: rows });
        });
    });
});
